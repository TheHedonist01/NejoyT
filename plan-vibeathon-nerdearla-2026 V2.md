...
**Ventana de construcción:** 24/09/2026 15:00 UTC → 25/09/2026 15:00 UTC (12:00 a 12:00 hora Argentina)
**Estado del documento:** diseño previo. Ningún archivo de código de este proyecto se escribe antes del 24/09.

---

## 0. Advertencia de cumplimiento

Las bases dicen textualmente que los proyectos comenzados antes de las fechas no serán tenidos en cuenta. La línea que este documento respeta es:

| Se puede hacer antes | No se puede hacer antes |
|---|---|
| Diseñar arquitectura y escribir este documento | Escribir código del proyecto |
| Leer documentación y correr los quickstarts oficiales de Google (y descartarlos) | Crear el repositorio con el código |
| Preparar el entorno: Python, ffmpeg, CUDA, drivers, cuenta Devpost, API key | Dejar módulos "listos para pegar" |
| Descargar audios de prueba y pesos de modelos | — |
| Redactar el glosario técnico | — |
| Bocetar el guion del video | Grabar el video |

Diseñar no es construir. Pero el repo se crea el 24 y el primer commit es de ese día.
...
**Decisión 4 — Las salas tienen un ciclo de vida, no son un archivo estático que se lee una vez.**
`rooms.yaml` deja de ser la única fuente de verdad y pasa a ser la semilla: las salas que ya sabés que vas a necesitar se cargan solas al arrancar, pero el orquestador además expone una API para crear, detener y ajustar salas en caliente — necesario porque en una conferencia real una sala se agrega de improviso (se suma un track) o hay que cortarla porque el orador terminó antes. Cada sala tiene un estado explícito (`starting → active → degraded → stopping → stopped`, más `error`), y tanto el panel de admin como la vista de audiencia leen ese estado en vez de asumir que "si no hay eventos, algo se rompió". La API de control se escribe en la Fase 5, porque es la misma pieza que te da las "dos sesiones en simultáneo" que piden las bases; el panel de admin de la Fase 6 es solo la cara visible de esa API.
...
### 1.5 Ingesta de audio: micrófono, streaming y archivo, en detalle

La Decisión 1 dice "todo entra por ffmpeg", pero las tres fuentes no son intercambiables en la práctica: cada una necesita un comando distinto, y dos de ellas (mic y streaming) no tienen fin natural, mientras que la tercera (archivo) hay que frenarla a propósito para que no corra más rápido que la charla real. `audio/source.py` no es un wrapper único, es una fábrica que arma el comando correcto según `source.kind`:

| Fuente | Comando ffmpeg (esquema) | Particularidad |
|---|---|---|
| **Micrófono** | `-f dshow -i audio="<dispositivo>"` (Windows) / `-f pulse -i default` (Linux) | Sin fin natural. La "señal de vida" es el propio audio: un silencio prolongado puede ser una pausa de la charla o el micrófono desenchufado. Hace falta un watchdog de silencio por umbral, no solo de proceso vivo. |
| **Streaming** (RTMP entrante desde un encoder tipo OBS/vMix, o HLS/`.m3u8` de un ingest ya existente) | `-f flv -listen 1 -i rtmp://0.0.0.0:1935/live/<room_id>` (ffmpeg escuchando) o `-i <url_hls>` (ffmpeg consumiendo) | Sin fin natural y con jitter de red. El caso realista de una conferencia es el primero: la mesa de sonido ya manda RTMP a un encoder, y tu servicio se para como receptor de esa señal en vez de andar buscando una URL pública. El HLS trae un delantero de varios segundos por diseño del protocolo — es un piso de latencia que no depende de vos y va documentado como limitación conocida, no como bug. |
| **Archivo** (demo y pruebas) | `-re -stream_loop -1 -i <archivo>` | El flag `-re` es el que importa: sin él, ffmpeg lee el archivo lo más rápido que puede y te rompe toda la lógica de "tiempo real" que estás probando. `-stream_loop -1` lo hace loopear, útil para dejar una sala de demo corriendo sola sin supervisión. |

**Dos piezas comunes a las tres, sin importar la fuente:**

- **Cola con backpressure.** Los chunks de ffmpeg entran a un `asyncio.Queue` de tamaño acotado (por ejemplo 50 chunks ≈ 5 s de buffer). Si el ASR se atrasa, la cola se llena y se descarta el chunk más viejo, no el más nuevo — preferís perder medio segundo de hace un rato que quedarte cada vez más atrás del audio real. Sin este límite, una sala lenta termina comiéndose la memoria del proceso en una charla de 45 minutos.
- **Supervisor de reconexión.** Un task aparte vigila el código de salida del subproceso de ffmpeg. Para mic y streaming (que no deberían terminar solos), una salida inesperada dispara reintento con backoff exponencial y jitter, y la sala pasa a estado `degraded` mientras tanto — no `error`, porque todavía puede volver sola. Para archivo, una salida limpia es el fin esperado de la prueba, no una falla.

La fábrica de fuentes se escribe entera en la Fase 1, aunque esa tarde solo la pruebes con archivo. Mic y streaming real (RTMP entrante) se terminan de validar en la Fase 5, cuando ya hay multi-sala, porque ahí es donde importa de verdad que dos fuentes vivas convivan sin pisarse.
...
- `audio/source.py`: fábrica de fuentes (mic / streaming / archivo) — ver sección 1.5. Cada modo arma su propio comando de ffmpeg pero todos convergen en la misma salida: iterador async de chunks PCM
  - Formato de salida común: `-f s16le -acodec pcm_s16le -ar 16000 -ac 1 -`, en chunks de 100 ms (1.024 a 2.048 frames), que es lo que recomienda la doc
  - `asyncio.Queue` acotada entre ffmpeg y el consumidor, con descarte del chunk más viejo si se llena
  - Supervisor de reconexión con backoff exponencial (probalo con archivo esta tarde; mic y RTMP entrante se validan en Fase 5, con multi-sala)
- `main.py`: FastAPI arriba, endpoint `/health`
- Prueba: levantar un archivo de audio con `-re` y loguear bytes por segundo. Debe dar ~32.000 B/s, constante.

**Criterio de salida:** ves bytes fluyendo a tasa constante desde un archivo (con `-re`) y desde una URL. El micrófono se certifica recién en Fase 5, junto con el resto de las fuentes en vivo.
...
### FASE 5 — Multi-sesión y control de salas (H+10 a H+12 · sáb 23:00-01:00)

**Objetivo:** el requisito mínimo formal (dos sesiones en simultáneo) más el ciclo de vida de sala que necesita cualquier panel de control.

- `orchestrator.py`: mantiene un diccionario de `SessionWorker` por sala, con estado explícito: `starting → active → degraded → stopping → stopped` (más `error`)
- `rooms.yaml` se carga al boot como semilla, no como única fuente de verdad
- API de control (el panel de admin de la Fase 6 es la cara visual de esto, por eso la API se escribe acá y no ahí):
  - `GET /api/rooms` — listado con estado, idiomas y métricas básicas de cada sala
  - `POST /api/rooms` — crea y arranca una sala nueva en caliente (fuente, idioma origen, idiomas destino)
  - `DELETE /api/rooms/{id}` — detiene una sala: corta ffmpeg, cierra el ASR, deja el estado en `stopped` y flushea lo acumulado a `store.py` para no perder el export
  - `PATCH /api/rooms/{id}` — cambia parámetros mutables sin reiniciar la sala entera: agregar/quitar un idioma destino, recargar el glosario. Cambiar el backend (cloud ↔ local) de una sala activa se resuelve reiniciando solo esa sala — no vale la complejidad de hacerlo en caliente en 24 h
  - Autenticación mínima: un token compartido por header (`X-Admin-Token`), aunque sea una sola variable de entorno. Es un panel expuesto en la misma red que el staff del evento, no puede quedar abierto
- Aislamiento de fallos: si una sala entra en `error`, las otras siguen; ese estado se refleja también en la vista de audiencia de esa sala ("la transcripción se retoma en breve") en vez de quedarse muda sin explicación
...
3. **Panel de admin** (~45 min) — `GET /admin`: tabla con las salas y su estado, y para cada una los controles que ya expone la API de la Fase 5 (detener, agregar/quitar idioma, recargar glosario), más las métricas (latencia p50/p95, reconexiones, contador de rotaciones, errores). Es la capa visual sobre una API que ya existe, por eso entra en 45 minutos y no en dos horas. Pega directo en "despliegue y operación": un jurado que ve que podés apagar una sala en vivo entiende de una que esto no es un script de demo.
